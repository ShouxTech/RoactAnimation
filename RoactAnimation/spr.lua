...
-- Modified by CriShoux for class support.
--
...
function spr.new(dampingRatio: number, frequency: number, initialValue: any)
	...
	local metadata = typeMetadata[typeof(initialValue)]
	if not metadata then
		error("unsupported type: " .. typeof(initialValue), 2);
	end;

	local self = setmetatable({}, spr);

	self._spring = metadata.springType(dampingRatio, frequency, initialValue, initialValue, metadata);
	self._spring:setGoal(initialValue)
	...
	self._updateConnection = RunService.PreSimulation:Connect(function(dt)
		if self._goalMet then return; end;

		if self._spring:canSleep() then
			self._goalMet = true;
			if self._onCompletedCallback then
				self._onCompletedCallback();
			end;
		...
		self.Value = self._spring:step(dt);
	end);
...
function spr:IsGoalMet()
	return self._goalMet;
end;

function spr:SetDampingRatio(dampingRatio: number)
	self._spring:setDampingRatio(dampingRatio);
end;

function spr:SetFrequency(frequency: number)
	self._spring:setFrequency(frequency);
end;

function spr:SetOnCompletedCallback(callback: () -> ())
	self._onCompletedCallback = callback;
end;

function spr:Destroy()
	if not self._updateConnection then return; end;
